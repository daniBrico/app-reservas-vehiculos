...
const defaultClasses =
  'bg-amber-900 dark:bg-stone-800 fixed z-[200] cursor-pointer items-center justify-center text-white dark:text-stone-400 shadow-xl duration-300 select-none'
...
const ScrollToTopButton = (): JSX.Element => {
  ...
  return (
    <>
      ...
      <button
        className={`${defaultClasses} right-8 bottom-32 hidden h-13 w-13 rounded-4xl shadow-sm transition-transform hover:scale-110 sm:flex dark:shadow-stone-950/80 ${showButton ? 'opacity-100' : 'pointer-events-none opacity-0'}`}
        onClick={handleButtonToTop}
      >
        <ArrowUpIconSvg />
      </button>
